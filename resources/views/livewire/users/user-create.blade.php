<div class="d-inline">
    <button type="button" class="btn btn-success mb-3" data-bs-toggle="modal" data-bs-target="#modalFormUser">
        @if(!request('role_id'))
            {{ __('Create user') }}
        @else
            {{ __('Create '. \Spatie\Permission\Models\Role::where('id', request('role_id'))->first()->name) }}
        @endif
    </button>

    <div wire:ignore.self class="modal fade" id="modalFormUser" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">@if(request('role_id')){{ __(\Spatie\Permission\Models\Role::where('id', request('role_id'))->first()->name.'s') }}@endif</h5>
